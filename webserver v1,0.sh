#!/bin/bash
#
# ФИНАЛЬНЫЙ СКРИПТ УСТАНОВКИ POWER-СТЕКА НА DEBIAN 12 (v1.0 - ПЕРВЫЙ СТАБИЛЬНЫЙ РЕЛИЗ)
# Эта версия включает все исправления: надежные сетевые операции и GPG-ключи,
# запасные варианты для API и единственно правильный метод настройки wp-config.php,
# чтобы избежать конфликтов с установщиком WordPress.
#
# --- ОТРЕДАКТИРУЙТЕ ЭТИ ПЕРЕМЕННЫЕ ПЕРЕД ЗАПУСКОМ ---
WP_DOMAIN="example.com"
PMA_DOMAIN="db.example.com"
...
MARIADB_FALLBACK_VERSION="11.4" # Запасная версия, если API недоступен
# Опции для curl: 3 попытки, 5с задержка, IPv4, тихий режим, ошибка при сбое, таймаут 15с
...
# Функция для получения последних версий
fetch_latest_versions() {
    echo -e "\n${YELLOW}Шаг 1: Добавление репозиториев и определение версий...${NC}"
    
    echo -e "${BLUE}Добавление GPG-ключей репозиториев...${NC}"
    ...
    echo -e "${BLUE}Обновление списков пакетов...${NC}"
    apt-get update

    # --- Определение версий с проверками ---
    ...
    MARIADB_LATEST_BRANCH=$(echo "$MARIADB_LATEST_DATA" | jq -r '.major_version' || true)
    
    if [ -z "$MARIADB_LATEST_BRANCH" ] || [ "$MARIADB_LATEST_BRANCH" == "null" ]; then
        MARIADB_LATEST_BRANCH=$MARIADB_FALLBACK_VERSION
        echo -e "${YELLOW}MariaDB: не удалось определить, используется запасная версия ${MARIADB_LATEST_BRANCH}.${NC}"
    else
        echo -e "${GREEN}MariaDB: ${MARIADB_LATEST_BRANCH}${NC}"
    fi
    ...
    WP_LATEST_DATA=$(curl $CURL_OPTIONS https://api.wordpress.org/core/version-check/1.7/)
    WP_LATEST=$(echo "$WP_LATEST_DATA" | jq -r '.offers[0].version')
    if [ -z "$WP_LATEST" ] || [ "$WP_LATEST" == "null" ]; then echo -e "${RED}Критическая ошибка: Не удалось получить версию WordPress.${NC}"; exit 1; fi
    echo -e "${GREEN}WordPress: ${WP_LATEST}${NC}"
    
    PMA_LATEST_DATA=$(curl $CURL_OPTIONS https://www.phpmyadmin.net/home_page/version.json)
    PMA_LATEST=$(echo "$PMA_LATEST_DATA" | jq -r '.version')
    if [ -z "$PMA_LATEST" ] || [ "$PMA_LATEST" == "null" ]; then echo -e "${RED}Критическая ошибка: Не удалось получить версию phpMyAdmin.${NC}"; exit 1; fi
    echo -e "${GREEN}phpMyAdmin: ${PMA_LATEST}${NC}"
}
...
echo -e "${GREEN}                  (Версия 1.0 - Первый Стабильный Релиз)${NC}"
echo -e "${BLUE}=====================================================================${NC}"

if [ "$WP_DOMAIN" == "example.com" ]; then echo -e "${RED}Пожалуйста, отредактируйте скрипт и укажите ваши реальные домены и email!${NC}"; exit 1; fi

echo -e "\n${YELLOW}Выберите действие:${NC}"
echo "1. Новая установка стека"
echo "2. Обновить системные пакеты на существующем сервере"
read -p "Введите номер (1 или 2): " ACTION

case $ACTION in
    1)
        # --- ПОЛНАЯ УСТАНОВКА ---
        echo -e "\n${YELLOW}Шаг 0: Подготовка системы и установка базовых утилит...${NC}"
        ...
        echo -e "\n${YELLOW}Настройка HTTP Basic Auth для phpMyAdmin (дополнительная защита):${NC}"
        ...
        if [ -z "$PMA_AUTH_USER" ] || [ -z "$PMA_AUTH_PASS" ]; then echo -e "${RED}Имя пользователя или пароль для phpMyAdmin не могут быть пустыми. Прерывание.${NC}"; exit 1; fi
        
        DB_USER_PASS=$(openssl rand -base64 16)

        echo -e "\n${YELLOW}Шаг 2: Установка компонентов (Nginx, Varnish, Redis, MariaDB, PHP)...${NC}"
        ...
        echo -e "\n${YELLOW}Шаг 3: Настройка PHP для логирования ошибок...${NC}"
        PHP_INI_PATH="/etc/php/${PHP_LATEST}/fpm/php.ini"
        sed -i 's/display_errors = Off/display_errors = On/' "$PHP_INI_PATH"
        sed -i 's/log_errors = Off/log_errors = On/' "$PHP_INI_PATH"
        sed -i "s|;error_log = .*|error_log = /var/log/php${PHP_LATEST}-fpm.log|" "$PHP_INI_PATH"
...
EOF
        ...
        echo -e "\n${YELLOW}Шаг 5: Установка WordPress и настройка wp-config.php...${NC}"
        WP_CONFIG_PATH="/var/www/${WP_DOMAIN}/wp-config.php"
        mkdir -p /var/www/${WP_DOMAIN}; cd /var/www/${WP_DOMAIN}
        ...
        cp wp-config-sample.php "$WP_CONFIG_PATH";
        sed -i "s/database_name_here/wordpress/g" "$WP_CONFIG_PATH";
        sed -i "s/username_here/wp_user/g" "$WP_CONFIG_PATH";
        sed -i "s#password_here#${DB_USER_PASS}#g" "$WP_CONFIG_PATH"
        ...
        # --- ФИНАЛЬНЫЙ БЛОК ДЛЯ WP-CONFIG ---
        # Мы добавляем этот блок сразу. Это предотвращает попытки установщика WordPress
        # добавить свои собственные определения, что вызывало ошибку "Constant already defined".
        CONFIG_CODE="\n/**\n * Настройки для работы за обратным прокси-сервером (Varnish/Nginx).\n */\n// 1. Установка адресов сайта, чтобы WordPress не пытался их переопределить.\ndefine( 'WP_HOME', 'https:\/\/${WP_DOMAIN}' );\ndefine( 'WP_SITEURL', 'https:\/\/${WP_DOMAIN}' );\n\n// 2. Исправление определения SSL, когда HTTPS терминируется на прокси.\nif ( isset(\\\$_SERVER['HTTP_X_FORWARDED_PROTO']) \&\& \\\$_SERVER['HTTP_X_FORWARDED_PROTO'] === 'https' ) {\n    \\\$_SERVER['HTTPS'] = 'on';\n}\n"
        # Вставляем наш код прямо перед строкой "That's all, stop editing!".
        sed -i "/\/\* That's all, stop editing! Happy publishing. \*\//i ${CONFIG_CODE}" "$WP_CONFIG_PATH"

        chown -R www-data:www-data /var/www/${WP_DOMAIN}
        mkdir -p /var/www/${PMA_DOMAIN}; cd /var/www/${PMA_DOMAIN}
        ...
        chown -R www-data:www-data /var/www/${PMA_DOMAIN}

        echo -e "\n${YELLOW}Шаг 6: Настройка Nginx, SSL и Basic Auth...${NC}"
        mkdir -p /etc/nginx/ssl
        htpasswd -cb /etc/nginx/.htpasswd-pma "${PMA_AUTH_USER}" "${PMA_AUTH_PASS}"
        openssl req -x509 -nodes -newkey rsa:2048 -days 365 -keyout /etc/nginx/ssl/dummy.key -out /etc/nginx/ssl/dummy.pem -subj "/C=US/ST=none/L=none/O=none/CN=localhost"
...
server { listen 80; server_name ${WP_DOMAIN}; return 301 https://\$host\$request_uri; }
...
server { listen 80; server_name ${PMA_DOMAIN}; return 301 https://\$host\$request_uri; }
...
EOF
        sed -i "s|/etc/letsencrypt/live/${WP_DOMAIN}/fullchain.pem|/etc/nginx/ssl/dummy.pem|g" /etc/nginx/conf.d/main-stack.conf
        sed -i "s|/etc/letsencrypt/live/${WP_DOMAIN}/privkey.pem|/etc/nginx/ssl/dummy.key|g" /etc/nginx/conf.d/main-stack.conf
        sed -i "/include \/etc\/letsencrypt\/options-ssl-nginx.conf;/d" /etc/nginx/conf.d/main-stack.conf
        sed -i "/ssl_dhparam \/etc\/letsencrypt\/ssl-dhparams.pem;/d" /etc/nginx/conf.d/main-stack.conf
        nginx -t

        echo -e "\n${YELLOW}Шаг 7: Исправление прав, получение SSL...${NC}"
        usermod -aG www-data nginx; systemctl restart php${PHP_LATEST}-fpm.service
        certbot --nginx -d ${WP_DOMAIN} -d ${PMA_DOMAIN} --non-interactive --agree-tos -m ${LETSENCRYPT_EMAIL} --redirect
        ...
        echo -e "${YELLOW}Пароль root для MariaDB вы установили самостоятельно.${NC}"
        echo -e "Пользователь БД для WP:         ${GREEN}wp_user${NC}"; echo -e "Пароль пользователя БД для WP:  ${GREEN}${DB_USER_PASS}${NC}"
    ...
    2)
        # --- ОБНОВЛЕНИЕ ---
        echo -e "\n${YELLOW}Обновление системных пакетов (Nginx, PHP, MariaDB, Redis...)${NC}"
        apt-get update
        apt-get dist-upgrade -y
        apt-get autoremove -y
        echo -e "\n${GREEN}Системные пакеты были обновлены до последних версий в репозиториях.${NC}"
        echo -e "${YELLOW}ВАЖНО: Этот режим не обновляет ядро WordPress или phpMyAdmin.${NC}"
        ;;
    *)
        echo -e "${RED}Неверный выбор. Прерывание.${NC}"
        exit 1
        ;;
esac
